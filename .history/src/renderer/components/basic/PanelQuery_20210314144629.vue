...
export default {
  ...
  computed: {
    bodyStyle() {
      ...
      let height = 0
      if (queryBar) console.info('style-height-1:', this.$refs['query-bar'].offsetHeight)
      // height += this.$refs['query-bar'].offsetHeight
      if (footer) height += this.$refs['footer'].offsetHeight
      console.info('style-height-2:', height)
