          ...
          this.$nextTick(() => {
            ...
            oneIframe.addEventListener('did-stop-loading', () => {
              ...
              oneIframe.executeJavaScript(
                `const sponsored = document.querySelectorAll('[tpl="softdown"],[class*="EC_"]'); for(let i = 0; i < sponsored.length; i++) { const item = sponsored[i]; item.style.display = 'none'; }`
              )
              // setTimeout(() => {
              //   oneIframe.executeJavaScript(
              //     [
              //       `const sponsored = document.querySelectorAll('._2z1q32z'); for(let i = 0; i < sponsored.length; i++) { const item = sponsored[i]; item.style.display = 'none'; }`,
              //       `const sponsored2 = document.querySelectorAll('._8pnr6st'); for(let i = 0; i < sponsored2.length; i++) { const item = sponsored2[i]; item.style.display = 'none'; }`,
              //       `const sponsored3 = document.querySelectorAll('[tpl="se_com_default"]'); for(let i = 0; i < sponsored3.length; i++) { const item = sponsored3[i]; console.info('result-item:', item); }`,
              //     ].join(' ')
              //   )
              // }, 1500)
