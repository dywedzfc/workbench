  ...
  watch: {
    visible: {
      handler(value) {
        ...
        if (value) {
          this.$nextTick(() => {
            const { oneIframe, twoIframe } = this.$refs
            // //开始加载事件监听
            ...
            twoIframe.addEventListener('did-stop-loading', () => {
              ...
              const styleList = [
                ...
                '@media (max-width: 1274.9px) { body { min-width: 680px !important; } }',
                '#b_content { padding-left: 30px; }',
                '#b_header, #b_content .ev_talkbox_wrapper, #ev_talkbox_wrapper, aside[aria-label="更多结果"], aside[aria-label="Additional Results"], #b_footer { display: none; }',
                '#b_results .b_rc_gb_template .b_widgetContainer { left: calc(100% + 127px); padding-left: 0; padding-right: 8px; border-radius: 0 15px 15px 0; }',
                '#b_results .b_rc_gb_w_content { padding: 8px 8px 8px 0; border-radius: 0 10px 10px 0; }',
                '#b_results .b_rc_gb_template .b_rc_gb_widget_link:hover { border-radius: 0 6px 6px 0; }',
                '#b_results .b_widgetGrad { left: 0; background: linear-gradient(90deg,rgba(0,0,0,.15) -68.75%,rgba(0,0,0,0) 81.25%); }',
                '#wrapper #s_tab { width: auto; padding-top: 15px; padding-left: 30px; }',
                '#wrapper #container.sam_newgrid { box-sizing: border-box; width: auto; margin: 0; padding-left: 30px; }',
                '#wrapper #container.sam_newgrid #content_right { display: none; }',
                '#wrapper .rs-normal-width_2T91A { width: 100%; }',
              ...
              // twoIframe.executeJavaScript('console.info("ceshi")')
