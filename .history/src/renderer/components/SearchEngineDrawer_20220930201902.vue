<template>
  <dialog-drawer :display.sync="search.display" type="search">
    ...
    <div class="search-list" v-if="search.display">
      <webView
        class="search-iframe"
        :src="`https://www.baidu.com/s?wd=${search.query}&tn=simple#`"
        allowpopups
        ref="oneIframe"
      ></webView>
      <webView
            ...
            oneIframe.addEventListener('did-stop-loading', () => {
              ...
              oneIframe.executeJavaScript(
                [
                  ...
                  `const sponsored3 = document.querySelectorAll('[id="1"]'); for(let i = 0; i < sponsored3.length; i++) { const item = sponsored3[i]; console.info('result-item:', item)  }`,
                ].join(' ')
              )
