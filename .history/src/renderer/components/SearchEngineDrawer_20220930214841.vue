        ...
        if (value) {
          this.$nextTick(() => {
            ...
            oneIframe.addEventListener('did-stop-loading', () => {
              ...
              const script = `const sponsored = document.querySelectorAll('[tpl="softdown"],[class*="EC_"]'); for(let i = 0; i < sponsored.length; i++) { const item = sponsored[i]; item.style.display = 'none'; }`
