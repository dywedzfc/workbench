    ...
    visible: {
      handler(value) {
        ...
        if (value) {
          ...
          xhr.onload = ({ target: { status, responseText } }) => {
            console.info('data-mounted:', status, responseText)
            if (status === 200) {
              this.$nextTick(() => {
                ...
                oneIframe.addEventListener('did-stop-loading', () => {
                  ...
                  oneIframe.insertCSS('.search-iframe #head { display: none; }')
                  oneIframe.executeJavaScript('console.info("ceshi")')
                  console.info('data-load:', status, responseText)
