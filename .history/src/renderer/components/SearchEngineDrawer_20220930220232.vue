        ...
        if (value) {
          this.$nextTick(() => {
            ...
            oneIframe.addEventListener('did-stop-loading', () => {
              ...
              this.initIframePreload(oneIframe, 'baidu')
              // _2z1q32z
              // const styleList = [
              ...
              // const script = `const sponsored = document.querySelectorAll('[tpl="softdown"],[class*="EC_"],._8pnr6st'); for(let i = 0; i < sponsored.length; i++) { const item = sponsored[i]; item.style.display = 'none'; }`
              // oneIframe.executeJavaScript(script)
            ...
            twoIframe.addEventListener('did-stop-loading', () => {
              console.log('did-stop-loading...')
              this.initIframePreload(twoIframe, 'bing')
              // const styleList = [
              ...
              //   '.b_respl .b_adTop, .b_respl .b_adBottom, .b_respl .sb_adTA { display: none; }', // 过滤列表中的广告
            ...
            theIframe.addEventListener('did-stop-loading', () => {
              console.log('did-stop-loading...')
              this.initIframePreload(theIframe, 'sogou')
              // const styleList = [
  ...
  methods: {
    ...
    initIframeData(type) {
      ...
      if (type == 'baidu') {
        ...
        data.script = `const sponsored = document.querySelectorAll('[tpl="softdown"],[class*="EC_"],._8pnr6st'); for(let i = 0; i < sponsored.length; i++) { const item = sponsored[i]; item.style.display = 'none'; }`
      } else if (type == 'bing') {
        data.style = [
          'html, body { min-width: 680px !important; }',
          '.b_respl #b_header { min-width: 100%; }',
    ...
    initIframePreload(iframeTag, type) {
      const { style, script } = this.initIframeData(type)
      if (style) iframeTag.insertCSS(style)
      if (script) iframeTag.executeJavaScript(script)
    },
  },
}
</script>
