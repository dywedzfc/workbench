<template>
  <div :class="['nav-box', { show }]">
    ...
    <div class="nav-wrapper">
      <template v-for="item in menu">
        <div :class="['link', menuActive(item)]" v-if="item.type == 'open'" :key="item.id">
          ...
          <span class="title">{{ item.hotKey }}</span>
    ...
    <div class="nav-footer">
      <template v-for="item in footMenu">
        <div :class="['link', menuActive(item)]" v-if="item.type == 'open'" :key="item.id">
          <i :class="['icon', iconClassName(item.icon)]"></i> {{ item.hotKey }}
        </div>
        <router-link :class="['link', menuActive(item)]" v-else :key="item.id" :to="item.href">
          <i :class="['icon', iconClassName(item.icon)]"></i> {{ item.title }}
        </router-link>
      </template>
    </div>
  </div>
