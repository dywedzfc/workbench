  ...
  computed: {
    ...
    _dailyHistoryTotal: {
      ...
      set(value) {
        ...
        const startSlice = historyList - value
        console.info('history-total:', historyList, startSlice)
        if (startSlice >= 0) {
          const resetList = historyList.slice(startSlice)
          localStorage.setItem('dailyHistory', JSON.stringify(resetList))
        }
