...
import { msgSuccess, msgWarning, messageBox, PROTOCOL, verifyServerAddress } from 'util'
  ...
  methods: {
    ...
    handleServerAddressCommandClick(url) {
      if (verifyServerAddress(url)) {
        console.info('address:', RegExp(PROTOCOL).test(url))
        // console.info('address:', url.includes(/^((http:\/\/)|(https:\/\/))/))
