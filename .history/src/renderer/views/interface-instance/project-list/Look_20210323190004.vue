    ...
    <div class="template-wrapper">
      <el-form class="panel-form" inline label-width="90px">
        ...
        <el-form-item>
          ...
          <path-box-content :url="form.serverAddress" :memo="form.serverAddressMemo" />
          <!-- <div class="input-box input-block" :class="{ 'input-url': form.serverAddressMemo }">
            {{ form.serverAddress }}
          </div>
          <div class="input-box input-block input-textarea" v-if="form.serverAddressMemo">
            {{ form.serverAddressMemo }}
          </div> -->
        </el-form-item>
        <el-form-item label="本地地址">
          <template #label>
            <span>服务器地址</span>
            <span class="input-label" v-if="form.localAddressMemo">说明</span>
          </template>
          <div class="input-box input-block" :class="{ 'input-url': form.localAddressMemo }">
            {{ form.localAddress }}
          </div>
          <div class="input-box input-block input-textarea" v-if="form.localAddressMemo">
            {{ form.localAddressMemo == true }}
          </div>
...
export default {
  name: 'ProjectEdit',
  components: { Page, PathBoxContent }, // , Panel
  data() {
    return {
      update: false,
      type: '',
