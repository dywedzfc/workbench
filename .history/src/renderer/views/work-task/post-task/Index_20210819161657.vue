  ...
  <page title="发布任务" icon="el-icon-s-home" @back="handlePageBack">
    <panel-query>
      <el-form slot="query-bar" class="query-form" inline>
        ...
        <el-form-item>
          <el-button type="primary" @click="handleAddClick">添加任务</el-button>
        </el-form-item>
      </el-form>
      <card-list width="600px">
        <card-item class="card-box" v-if="postTask.type == 'add'">
          <el-form class="form-card" label-width="80px">
            ...
            <el-form-item label="备注">
              <el-input
                class="input-block"
                v-model.trim="postTask.add.remarks"
                type="textarea"
                rows="3"
              />
            </el-form-item>
            <el-form-item class="form-item__col" label="优先级">
              <el-select class="input-block" v-model.trim="postTask.add.state">
                <el-option
            ...
            <el-form-item class="form-item__col" label="截止日期">
              <el-input class="input-block" v-model.trim="postTask.add.deadline" />
            </el-form-item>
            <el-form-item class="form-center" label-width="0">
              <el-button @click="handleAddPostTaskCancelClick">取消</el-button>
              <el-button type="primary" @click="handleAddPostTaskSubmitClick">保存</el-button>
            </el-form-item>
          </el-form>
        </card-item>
        <card-item class="card-box" v-for="item in postTaskList" :key="item.id" :state="item.state">
...
export default {
  ...
  computed: {
    ...mapGetters({ projectList: PROJECT_OPTIONS, postTaskOptions: POST_TASK_OPTIONS }),
    // projectOptions() {
  ...
  methods: {
    ...
    handleQueryClick() {},
    handleAddClick() {
      this.postTask.type = 'add'
    },
    handleAddPostTaskCancelClick() {
      this.postTask.type = ''
      this.postTask.add.projectId = ''
      this.postTask.add.task = []
      this.postTask.add.task.push({ value: '' })
    },
    handleAddPostTaskSubmitClick() {
...
.form-item__col::v-deep.el-form-item {
  display: inline-block;
  width: 50%;
}

.form-center::v-deep.el-form-item {
  padding: 10px;
  padding-top: 15px;
  text-align: center;
}
