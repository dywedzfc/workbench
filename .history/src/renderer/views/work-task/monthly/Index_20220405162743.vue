...
export default {
  ...
  data() {
    return { loading: false, query: { name: '', year: '' }, type: { state: false, year: '' } }
  },
  computed: {
    ...mapGetters({ projectList: PROJECT_OPTIONS, monthly: MONTHLY_INFO }),
    filterMonthlyList() {
    ...
    currentYear() {
      return this.type.year || new Date().getFullYear()
    },
    taskType() {
      return this.type.state ? '月' : '年'
    },
  },
  watch: {
    'query.year'() {
  ...
  methods: {
    getMonthlyInfo() {
      ...
      this.type.year = year || new Date().getFullYear()
    ...
    handleTypeClick() {
      // if (this.type.state == 'year') this.type.state = 'month'
      // else
      // this.type.state = this.type.state == 'year' ? 'month' : 'year'
      this.type.state = !this.type.state
    },
  },
}
</script>
