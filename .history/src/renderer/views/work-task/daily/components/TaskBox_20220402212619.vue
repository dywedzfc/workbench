...
<template>
  <div class="task-box">
    ...
    <span class="task-count">{{ data.task.split('|:|').length }}</span>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    projectButtonText() {
      if (this.title.type && !this.projectTitleName) this.$set(this.title, 'type', false)
      console.info('data-text:', this.title.type, this.projectInfo)
  ...
  methods: {
    ...
    handleTitleNameClick() {
      console.info('data:', this.title.type)
      if (!this.hasJianName) return (this.title.type = false), undefined
    ...
    handleTaskEditSubmitClick(item, index) {
      ...
      const data = { data: _.cloneDeep(this.data) }
      data.data.task = _.map(taskList, (v, i) => (v == item && i == index ? value : v)).join(split)
      data.query = { date: this.date, projectId: this.data.projectId }
