...
export default {
  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      const list = this.resetElementData(data.default, obj, 'root', base)
    ...
    resetElementData(data, type, path, base) {
      ...
      return _.map(data, (item, index) => {
        ...
        if (item.children)
          td.children = this.resetElementData(item.children, type, (path += '/' + index), base)
        return td
      })
    },
