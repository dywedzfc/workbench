    ...
    resetElementTag() {
      ...
      if (structure) {
        ...
        this.result.data = _.map(list, item => {
          const name = item.tag.substring(item.tag.indexOf('-') + 1)
          console.info('item:', name, item.options, data[active])
          item.options.on = Object.assign(item.options.on, data[active].on)
        })
      } else {
        const list = this.resetElementData(data[active].structure, 'root')
        this.result.data = list
