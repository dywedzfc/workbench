...
export default {
  ...
  methods: {
    vueHtml(item, level, index) {
      ...
      const inputValue = options.props.value ? true : false
      return h(
        item.tag,
        {
          class: options.class,
          props: inputValue? Object.assign(options.props,{value:this.query[this.options.value]}),
    ...
    vueElementOptions(item) {
      ...
      _.each(item.options.props, (value, key) => {
        if (key == 'v-model') {
          // props.value = this.query[value]
          props.value = value
          on.input = v => (this.query[value] = v)
