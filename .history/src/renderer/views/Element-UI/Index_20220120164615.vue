  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      this.right.data = _.map(options.data, item => {
        ...
        // _.map( options[item.name], item => (item.default != null ? (item.value = item.default) : undefined, item) )
        return item
      })
      ...
      _.each(options.el, item => {
        const data = _.filter(this.right.data, it => it.state == item)
        console.info('obj-data:', data, item)
        // if (data.length) obj[data[0].state] = data[0]
        const o = {}
        _.each(data, item => (o[item.type] = data))
        obj[item] = o
        obj[item].state = 'button'
        obj[item].tag = 'el-button'
        obj[item].title = '基础属性'
        // _.each(data, item => {})
        if (data.length) obj[data[0].state] = data[0]
      })
      console.info('data-init:', list, options, obj)
    ...
    resetElementData(data, path) {
      ...
      return _.map(data, (item, index) => {
        ...
        // const { base } = this.result.data
        _.each(this.right.data, item => {
          console.info('data-item:', item, it)
          if (item.type == 'base') {
