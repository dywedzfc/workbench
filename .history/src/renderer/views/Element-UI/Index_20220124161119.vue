...
export default {
  ...
  methods: {
    ...
    resetElementTag() {
      ...
      const { data, active } = this.guide
      const { data, active } = this.result
      // const list = this.resetElementData(data[active].structure, 'root')
      // this.result.data = list
      console.info('data-list:', this.guide.data[this.guide.active])
      return _.map(this.guide.data[this.guide.active].structure, item => {
        const list = this.resetElementData(this.result.tagData[item.state], 'root')
        console.info('data-list-1:', item.state, list)
        // return td
      })
      // if (structure) {
      //   const list = this.resetElementStructureData(structure)
