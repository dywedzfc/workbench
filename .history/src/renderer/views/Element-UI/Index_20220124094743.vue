  ...
  methods: {
    ...
    resetElementStructureData(structure) {
      ...
      return _.map(data[active].structure, (item, index) => {
        ...
        console.info('data-it2:', structure, options, item)
        td.path = 'root/' + index
        td.options = {}
        td.options.class = []
        td.options.props = {}
        ...
        console.info('data-it3:', options.tagData[item.state], data)
        if (data) {
          td.tag = data.tag
          td.rawData.base = datat.base
          td.rawData.attributes = datat.attributes
          //   _.each(it.base, item => {
          //     // if (item.name == 'text') td.options.domProps.innerText = item.value
          //     if (item.name == 'text') td[item.name] = item.value
          //     else if (item.name == 'class') td.options.class = _.filter(item.value.split(' '))
