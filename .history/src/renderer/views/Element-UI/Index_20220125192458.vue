    ...
    writeEvent(value, key) {
      ...
      return () => {
        ...
        if (active == 'messageBox') {
          ...
          let title = ''
          _.each(list, item => {
            console.info('data-each-item:', item.name, item.value, item.type, typeof item.value)
            if (this.hasTypeEmpty(item.value)) {
          ...
          if (Object.entries(data).length) {
            if (title) propsText = `'${message}','${title}', ${JSON.stringify(data)}`
            else propsText = `'${message}', ${JSON.stringify(data)}`
          } else {
