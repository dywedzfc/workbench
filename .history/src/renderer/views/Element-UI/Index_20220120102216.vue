      ...
      <div class="panel-body">
        <el-collapse v-model="collapse.active">
          <el-collapse-item title="基础属性" name="baseProps">
            <el-form class="container-form" label-width="120px">
              <el-form-item
                label-width="120px"
                :label="item.name"
                v-for="item in right.base"
                :key="item.name"
              >
...
export default {
  ...
  computed: {
    baseAttributesList() {
      const { query } = this.right
      const data = this.result.options
      console.info('data-el:', data, query)
  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      console.info('data-right:', this.right.base)
      const obj = {}
      _.each(data.el, item => {
      ...
      const list = this.resetElementData(data.default, obj, 'root')
    ...
    resetElementData(data, type, path) {
      ...
      return _.map(data, (item, index) => {
        ...
        if (item.children)
          td.children = this.resetElementData(item.children, type, (path += '/' + index))
        console.info('data:', td)
        return td
      })
    },
