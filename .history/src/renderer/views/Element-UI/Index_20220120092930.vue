...
export default {
  ...
  computed: {
    baseAttributesList() {
      const { el } = this.result.options
      console.info('data-el:', el)
      return [].concat([
  ...
  methods: {
    handleElementTagClick(item, index) {
      let path = ''
      ...
      if (path.length) {
        ...
        //   console.info('element-tag:', index == path[0], this.result.data)
        //   if (children.length) item.children = children
        //   return item
        // })
      }
      console.info('data-ET:', item, path, path.length)
