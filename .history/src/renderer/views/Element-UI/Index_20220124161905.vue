  ...
  methods: {
    ...
    resetElementStructureData(structure) {
      ...
      return _.map(data[active].structure, (item, index) => {
        ...
        td.path = 'root/' + index
        console.info('data-it2:', structure, td)
        return td
      })
    },
    resetElementTagData(data) {
      ...
      // const data = options.tagData[item.state]
      console.info('data-td:', data)
      if (data) {
        td.tag = data.tag
    ...
    resetElementTag() {
      ...
      const { data, active } = this.guide
      const { tagData } = this.result
      // const list = this.resetElementData(data[active].structure, 'root')
      // this.result.data = list
      console.info('data-list:', this.guide.data[this.guide.active])
      const list = this.resetElementStructureData()
      // return _.map(data[active].structure, item => {
      //   // const list = this.resetElementData(tagData[item.state], 'root')
      //   console.info('data-list-1:', item.state, tagData[item.state])
      //   // return td
      // })
      // if (structure) {
      //   const list = this.resetElementStructureData(structure)
