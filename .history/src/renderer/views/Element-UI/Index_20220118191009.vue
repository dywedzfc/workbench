  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      this.right.data = _.map(data.data, item => {
        ...
        item.data = _.map(data[item.name], item => ((item.value = item.default), item))
        return item
      })
      const obj = {}
      _.each(data.el, item => {
      ...
      const vm = new Vue({
        render(h) {
          // return h(`el-row`, {}, [h('el-col')])
          return _.map(list, item => {
            // item.children ? _.map(item.children, item => vh(item)) : undefined
            const vh = item => {
              return h(item.name, { props: item.prop })
            }
            return vh(item) // h(item.name, { props: item.prop })
          })
        }
      }).$mount()
    ...
    resetElementData(data, type) {
      return _.map(data, item => {
        ...
        td.rawData = it.data
        _.each(it.data, item => {
          td.prop[item.name] = item.value
          td.defaultProp[item.name] = item.default
        })
        if (it.children) td.children = this.resetElementData(td.children, type)
        console.info('data:', it)
        return td
      })
    },
