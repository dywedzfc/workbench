...
export default {
  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      const vm = new Vue({
        render(h) {
          const el = _.map(list, item => {
            const vh = item => {
              const children = item.children ? _.map(item.children, item => vh(item)) : undefined
              return h(item.tag, { props: item.props }, children)
            }
            return vh(item) // h(item.name, { props: item.props })
          })
          el.push(h('el-button', { class: ['button-add'], props: { icon: 'el-icon-plus' } }))
          return h('div', el)
        }
      }).$mount()
      // console.info('data-aaaa:', vm.$el)
      // _.each(vm.$el, item => this.$refs['panel-result'].appendChild(item))
      this.$refs['panel-result'].appendChild(vm.$el)
      // const addButton = new Vue({
      //   render(h) {
      //     return h('el-button', { class: ['button-add'], props: { icon: 'el-icon-plus' } })
      //   }
      // }).$mount()
      // this.$refs['panel-result'].appendChild(addButton)
    },
