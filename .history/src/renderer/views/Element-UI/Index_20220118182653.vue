...
export default {
  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      const obj = {}
      _.each(data.el, item => {
        const data = _.filter(this.right.data, it => it.type == item)
        console.info('data:', data, this.right.data)
        if (data.length) obj[data[0].type] = data[0]
      })
      // const row = _.filter(this.right.data, (item = item.type == 'row'))
      // const col = _.filter(this.right.data, (item = item.type == 'col'))
