...
export default {
  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      this.guide.active = item.id && item.id[0].toLowerCase() + item.id.substring(1)
      const { data, active } = this.guide
      console.info('data:', active, data)
    ...
    getFileName(name){
      return item.id ? item.id[0].toLowerCase() + item.id.substring(1):undefined
    }
    getElementTag(name) {
