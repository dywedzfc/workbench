...
export default {
  ...
  methods: {
    ...
    resetElementTagName(options) {
      ...
      console.info('data-tag:', rightData)
    ...
    getFileName(name) {
      console.info('data:', name[0].toLowerCase() + name.substring(1))
    ...
    writeEvent(value, key) {
      ...
      return () => {
        ...
        _.each(list, item => item.value && (data[item.name] = item.value))
        let vdata = ''
        if (this.guide.active == 'messageBox') {
          const message = data.message
          delete data.message
          const props = ''
          if (Object.entries(data).length) props = `${message}, ${JSON.stringify(data)}`
          else vdata = value.replace(/(\${props})/, `${message}`)
          vdata = value.replace(/(\${props})/, `${message}, ${JSON.stringify(data)}`)
          console.info('data-0:', message, vdata)
        } else {
          vdata = value.replace(/(\${props})/, JSON.stringify(data))
        }
        if (vdata) eval(vdata)
      }
    }
