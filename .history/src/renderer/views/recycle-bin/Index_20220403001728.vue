  ...
  <page title="回收站" icon="el-icon-s-home" @back="handlePageBack">
    <panel-query>
      ...
      >
        ...
        <el-table-column label="路径" width="120" v-bind="tableColProps">
...
export default {
  ...
  computed: {
    ...mapGetters({ recycleList: RECYCLE_LIST }),
    tableColProps() {
      return { align: 'center' }
    },
    filterTableList() {
      const { name } = this.query
      return _.filter(this.recycleList, (item) => name == '' || item.title.includes(name))
    },
  },
  mounted() {
    requestRecycleList()
