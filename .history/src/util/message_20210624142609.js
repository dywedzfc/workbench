...
export function msgWarning(msg, type) {
  const flag = msg == '录入失败'
  if (flag && ['A', 'add', 'append'].includes(type)) return msgWarning('添加失败')
  if (flag && ['U', 'upd', 'update'].includes(type)) return msgWarning('修改失败')
  if (flag && ['D', 'del', 'delete'].includes(type)) return msgWarning('删除失败')
