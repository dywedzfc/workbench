...
export function msgWarning(msg, type) {
  const flag = msg == '录入失败'
  if (['A', 'add', 'append'].includes(type)) return msgWarning(flag ? '添加失败' : msg)
  if (['U', 'upd', 'update'].includes(type)) return msgWarning(flag ? '修改失败' : msg)
  if (['D', 'del', 'delete'].includes(type)) return msgWarning(flag ? '删除失败' : msg)
