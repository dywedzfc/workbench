...
export function getProjectFiles(fileName) {
  ...
  console.info('ffffff-file:', fileName)
  const fileList = []
...
export function setProjectFiles(data, pFileName) {
  ...
  // const fileInfo = { data, count: count || maxNumber }
  fs.writeFile(`${path}${pFileName}${fileName}`, JSON.stringify(data), 'utf8', function(error) {
    if (error) return msgWarning('录入失败')
    msgSuccess('录入成功')
  })
}
