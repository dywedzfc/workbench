...
import { msgWarning, messageBox } from 'util'
...
export default {
  ...
  methods: {
    ...
    handleProjectClick() {
      ...
      requestAddProject({ title: this.query })
      setTimeout(requestProjectList)
    },
    ...
    handleTodoListRemove({ id, title }) {
      messageBox(`是否确认删除 "${title}"？`)
        .then(() => {
          requestRemoveProjectMark(id)
          setTimeout(requestProjectList)
        })
        .catch(() => {})
    }
