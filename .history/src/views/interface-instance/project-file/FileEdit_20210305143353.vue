  ...
  <page :title="`${tableFileItem.title}表文件`" @back="handlePageBack">
    <div class="template-wrapper">
      <div class="textarea-box">
        ...
        <el-button class="button-query" type="primary" @click="handleGenerateClick">生成字段</el-button>
        <el-button class="button-query" type="primary" @click="handleSubmitClick">保存</el-button>
        <el-button class="button-query" type="primary" @click="handleGenerateFileClick">生成文件</el-button>
      </div>
      <table class="table">
        <colgroup>
          <col width="200" />
        ...
        <tr class="table-row" v-for="item in props.data" :key="item.id">
          ...
          <td class="table-data table-data__need">
            <el-switch v-model="item.need" active-color="#13ce66" inactive-color="#ff4949"></el-switch>
          </td>
          <td class="table-data">
...
export default {
  ...
  methods: {
    ...
    setJSFile() {
      ...
      fs.readFile('./template/template.entity.js', 'utf8', (err, data) => {
        if (err) throw err
        const { fileName } = this.tableFileItem
        let configName = ''
        let className = ''
        if (fileName && fileName.split('-').length) {
          _.forEach(fileName.split('-'), (value, index) => {
        ...
        this.form.data.jsFile = content
        console.info('data:', content, this.form.data)
      })
    },
    setJSONFile() {
      const data = { props: {} }
      _.forEach(this.props.data, item => {
        if (!data.props[item.name]) data.props[item.name] = item
      })
