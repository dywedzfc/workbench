...
import { removeFile } from '../../../.history/src/server/folder_20210316184344'
...
function removeProjectItem(data) {
  ...
  if (tableName.code == 200) {
   ...
   cosnt maxCode =  _.max(_.map(removeList, 'code'))
   if(maxCode==400){}else {
    const removeFolder = removeFile(data.path)
    console.info('remove-folder:', removeFolder)
   }
    console.info('remove-list:', removeList)
  }
  console.info('remove-project:', data, project, tableName)
}
