...
export async function removeRecycleBinItem(id) {
  const data = getRecycleBinById(id)
  let state = {}
  console.info('project-data:', data)
  if (data.type == '项目文件') {
    state = await removeProjectItem(data)
    if (state.code == 200 || state.code == 300) return removeRecycleState(id)
    return state
  } else if (data.type == '项目表文件') state = await removeProjectTableItem(data)
...
async function removeProjectItem(data) {
  ...
  if (tableName.code == 200 && tableName.data.length) {
    const state = await removeProjectTableList(data.path, tableName.data)
    if (state.code == 400) return state
  }
  const hasFolder = await hasProjectNameFolder(data.path)
