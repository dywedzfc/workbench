...
const baseURL = process.env.NODE_ENV === 'production' ? `/workbench/` : 'src/'
...
export async function getProjectFolder() {
  ...
  try {
    if (!fs.statSync(configFile).isDirectory()) fs.mkdirSync(configFile)
    ...
    // return { error: error + '::判断' + process.execPath }
    try {
