...
export function getRecycleBin() {
  createFile(RECYCLE_FILE_NAME)
  const path = Path.join(baseURL, `${CONFIG_PATH}${RECYCLE_FILE_NAME}`)
  try {
    fs.accessSync(path)
  } catch (error) {
    try {
      const content = JSON.stringify({ data: [], count: 0 })
      fs.writeFileSync(path, content, 'utf8')
      console.log(`"${RECYCLE_FILE_NAME}"文件，写入成功！`)
    } catch (error) {
      console.error(`"${RECYCLE_FILE_NAME}"文件，写入失败！错误信息：(${error})`)
    }
  }
}

function createFile(fileName, pathFileName) {
  const path = Path.join(baseURL, `${CONFIG_PATH}${pathFileName}`)
  try {
    fs.accessSync(path)
  } catch (error) {
    try {
      ...
      console.error(`"${fileName}"文件，写入失败！错误信息：(${error})`)
    }
  }
}
