...
export function findProjectFileName(fileName) {
  if (!fileName) return {}
  const projectFile = Path.join(baseURL, `${CONFIG_PATH}${fileName}`)
  const projectNameList = fs.readdirSync(projectFile)
  const count = _.filter(projectNameList, item => item == fileName)
  if (count.length) return { code: 401, msg: '文件重复' }
  return { code: 200, msg: '无重复文件' }
}
