...
import { hasDailyHistory, getDailyHistory, setDailyHistory } from 'assets/script/local-storage'
import {
  DAILY_IS_HISTORY,
  DAILY_HISTORY_TOTAL,
...
export default {
  ...
  mounted() {
    console.info('data-mounted:', hasDailyHistory())
    if (!hasDailyHistory()) setDailyHistory([])
  ...
  methods: {
    ...
    projectOptions() {
      const options = []
      if (this.dailyIsHistory) {
        const historyList = getDailyHistory().reverse()
        const count = this.dailyHistoryShowNumber
        options.push({ label: '历史记录', options: historyList.slice(0, count) })
      }
      options.push({ options: this.originalProjectOptions })
      return options
    },
    setLocalStorage(projectId) {
      const list = _.filter(this.originalProjectOptions, (item) => item.value == projectId)
      ...
      if (historyList.length > this.dailyHistoryTotal) historyList.shift()
