...
<template>
  <div class="task-box">
    ...
    <span class="task-count" v-if="monthlyIsNumRecord">{{ data.task.length }}</span>
  </div>
</template>
...
import { PROJECT_OPTIONS, MONTHLY_IS_NUM_RECORD, requestProjectList } from 'assets/script/request'
...
export default {
  ...
  computed: {
    ...mapGetters({ projectList: PROJECT_OPTIONS, monthlyIsNumRecord: MONTHLY_IS_NUM_RECORD }),
    projectName() {
