  ...
  <page :title="`${tableFileItem.title}表文件`" @back="handlePageBack">
    <div class="template-wrapper">
      ...
      <table class="table" v-if="props.data.length">
        ...
        <tr class="table-row" v-for="(item, index) in props.data" :key="item.id">
          <td class="table-del">
            <el-button
              class="button-remove"
              icon="el-icon-remove"
              @click="headerRemoveRowClick(index)"
            ></el-button>
          </td>
          <td class="table-data">
          ...
          <td class="table-data table-data__need">
            <el-switch v-model="item.need"></el-switch>
            <!-- active-color="#13ce66" -->
...
export default {
  ...
  methods: {
    ...
    headerRemoveRowClick(index) {
      const item = this.props.data[index]
      this.$confirm(`此操作将永久删除"${item.sqlName}"数据, 是否继续?`, '提示', { type: 'warning' })
        .then(() => {
          // const index = _.indexOf(_.map(this.props.data, 'sqlName'), item.sqlName)
          this.$delete(this.props.data, index)
        })
        .catch(() => {})
    },
