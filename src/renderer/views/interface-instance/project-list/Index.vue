    ...
    <panel-query v-if="projectType == 'table'">
      <el-form slot="query-bar" class="query-form" inline>
        <el-form-item>
          <el-input
            v-model="query"
            placeholder="项目名称"
            @keyup.enter.native="handleProjectTableEnter"
          ></el-input>
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column prop="abbreviation" label="简称" min-width="140" v-bind="tableColProps">
          <template #default="{ row }">
            <template v-for="({ title, type }, index) in titleHighlight(row.abbreviation)">
              <template v-if="type == 'text'">{{ title }}</template>
              <span class="highlight" v-if="type == 'mark'" :key="index">{{ title }}</span>
            </template>
          </template>
        ...
        <el-table-column prop="fileName" label="文件名" min-width="160" v-bind="tableColProps">
          <template #default="{ row }">
            <template v-for="({ title, type }, index) in titleHighlight(row.fileName)">
              <template v-if="type == 'text'">{{ title }}</template>
              <span class="highlight" v-if="type == 'mark'" :key="index">{{ title }}</span>
            </template>
          </template>
...
export default {
  ...
  data() {
    return { query: '', confirmQuery: '', type: '' }
  },
  computed: {
    ...mapGetters({
      hotkeys: HOTKEYS,
    ...
    filterProjectList() {
      // const query = this.projectType == 'table' ? this.confirmQuery.trim() : this.query.trim()
      ...
      // return projectMultipleQueryFilter(query, projectList)
      const tableQuery = this.confirmQuery.trim()
      const cardQuery = this.query.trim()
      return this.projectType == 'table'
        ? projectMultipleQueryFilter(tableQuery, projectList)
        : multipleQueryFilter(cardQuery, projectList)
    },
    filterTableList() {
  ...
  methods: {
    ...
    handleQueryClick() {
      this.confirmQuery = this.query.toString()
      requestProjectList()
      this.tableCurrentPage = 1
    },
    handleAddClick() {
    ...
    handleProjectEnter() {
      if (this.query.includes(' ')) return
      if (this.query.trim() != '') this.handleProjectClick()
    },
    handleProjectTableEnter() {
      if (this.projectType == 'table') this.handleQueryClick()
    },
    handleProjectClick() {
