  ...
  <el-container class="container-wrapper">
    <el-container class="container-wrapper">
      <el-main class="container-body">
        <div class="panel-icon" v-if="right.query == 'Icon 图标'">
          <div
            class="icon-box"
            v-for="item in iconOptions"
            :key="item"
            @click="handleCopyToClip(item)"
          >
            <i :class="item"></i>
            <span>{{ item }}</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleDrawerButtonClick(item) {
      ...
      this.element.setDataOptions(_.cloneDeep(options))
      const { data, active } = this.guide

      const { data: rd, tagData } = this.resetElementTagName(options)
      if (structure) {
        this.element.setStructureOptions(_.cloneDeep(structure))
      ...
      console.info('click:', this.element, options)
     ...
     */
    handleCopyToClip(value) {
      // const copy = '<i class="' + value + '"></i>'
      console.info('data-copy:', value)
    ...
    resetElementTag() {
      ...
      if (structure) {
        const list = this.resetElementStructureData(structure)
        const resetList = _.map(list, (item) => {
