  ...
  <div :class="['nav-box', { show }]">
    ...
    <div class="nav-wrapper">
      <template v-for="item in menu">
        <div
          :class="['link', menuActive(item)]"
          v-if="item.type == 'open'"
          :key="item.id"
          @click="handleMenuOpenClick(item)"
        >
...
import { requestSetMenuShow } from 'assets/script/request'
import { getLeftMenu, setLeftMenu } from 'assets/script/local-storage'
...
export default {
  ...
  methods: {
    handleLogoButtonClick() {
      setLeftMenu({ show: (this.show = !this.show) })
      requestSetMenuShow(this.show)
    },
    handleMenuOpenClick(item) {
      // console.info('t-ui:', ipcRenderer)
      // ipcRenderer.invoke('open-browser', 'http://www.baidu.com')
      // if (item.id == 'TUI')
      ipcRenderer.invoke('open-window', item.id)
    },
    menuActive(item) {
