...
export function projectMultipleQueryFilter(query, list) {
    ...
    if (q.length > 1) {
      ...
      let hitList = _.map(q, (_q, i) => {
        ...
        if (title.includes(_q)) {
          ...
          data.hit = _q.length
        } else if (item.abbreviation && item.abbreviation.includes(_q)) {
          const startIndex = item.abbreviation.indexOf(_q)
          data.si = startIndex
          data.ei = startIndex + _q.length
          data.hit = _q.length
        } else if (item.fileName && item.fileName.includes(_q)) {
          const startIndex = item.fileName.indexOf(_q)
          data.si = startIndex
          data.ei = startIndex + _q.length
          data.hit = _q.length
        } else data.hit = 0
      ...
      if (
        ...
        (item.abbreviation && item.abbreviation.includes(query)) ||
        (item.fileName && item.fileName.includes(query))
      )
        data.hit = query.length
      else data.hit = 0
    }
    return data
  })
