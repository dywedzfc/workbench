...
const HOSTNAME_WEB = `([a-zA-Z0-9]+(\\.[a-zA-Z0-9]+)*(\\.[a-zA-Z]+))`
const PORT =
  ':([1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]{1}|6553[0-5])'
const PATH = '([a-zA-Z0-9_-]+\\/?)*'
const FILE_NAME = '(([a-zA-Z0-9_-]+)?.[a-z]*)?'

export function resetWebURL(url) {
  if (!RegExp(PROTOCOL).test(url)) url = `http://${url}`
  return url
}
export function verifyServerAddress(path = '') {
  const regExp = `^(${PROTOCOL})?((${HOSTNAME})|(${HOSTNAME_WEB}))((${PORT})?|((${PORT})\\/(${PATH})(${FILE_NAME})))$`
  const regUrl = RegExp(regExp)
  // if (/^([0-9]+)(\.([0-9]+)){1,2}$/.test(path)) return false
